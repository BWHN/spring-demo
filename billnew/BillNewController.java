package com.weiyuntong.module.bizcenter.controller.admin.billnew;
...
public class BillNewController {
    ...
    @PostMapping("/create-out-bill")
    @Operation(summary = "运单对账单-创建运单对账单")
    @PreAuthorize("@ss.hasPermission('bizcenter:bill-new:create')")
    @Idempotent
    public CommonResult<Boolean> createOutBill(@Valid @RequestBody List<Long> ids) {
        return success(billNewService.createOutBill(ids));
    }
     ...
     * @return
     */
    @GetMapping("/outPageCount")
    @Operation(summary = "运单对账单- 分页Tab统计")
    @PreAuthorize("@ss.hasPermission('bizcenter:bill-new:query')")
    public CommonResult<OutBillNewRespCountVO> getOutBillNewPageCount(@Valid OutBillNewPageReqVO pageVO) {
        return success(billNewService.getOutBillNewPageCount(pageVO));
    }
    @GetMapping("/outPage")
    @Operation(summary = "运单对账单- 分页")
    @PreAuthorize("@ss.hasPermission('bizcenter:bill-new:query')")
    public CommonResult<PageResult<OutBillNewRespVO>> getOutBillNewPage(@Valid OutBillNewPageReqVO pageVO) {
        return success(billNewService.getOutBillNewPage(pageVO));
    }
